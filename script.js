...
const contactPath = path.join(process.cwd(), 'web/components/band/contact-section.tsx');
...
const adminPath = path.join(process.cwd(), 'web/app/admin/page.tsx');
let acontent = fs.readFileSync(adminPath, 'utf-8');
...
;
acontent = acontent.replace("const [shows, setShows] = useState<Show[]>([])", settingsState + "\n  const [shows, setShows] = useState<Show[]>([])");
...
;
acontent = acontent.replace("const fetchShows = async () => {", fetchSettingsFunc + "\n  const fetchShows = async () => {");
...
;
acontent = acontent.replace("const handleLogout = () => {", saveSettingsHandler + "\n  const handleLogout = () => {");
...
;
acontent = acontent.replace("        {/* --- FORM DIALOG MODAL --- */}", settingsTab + "\n        {/* --- FORM DIALOG MODAL --- */}");
